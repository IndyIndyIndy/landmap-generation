...
use ChristianEssl\LandmapGeneration\Model\SurfaceMap;
...
class LandmapGenerator
{
     ...
     * @var float
     */
    protected $altitudeDifferenceWeight = 0.45; //@todo auslagern

    /**
     * @var float
     */
    protected $distanceDifferenceWeight = 0.035; //@todo auslagern
    ...
    public function __construct(GeneratorSettingsInterface $settings)
    {
        ...
        $this->surfaceMap = new SurfaceMap(
            $settings->getWidth(),
            $settings->getHeight()
        );
    }
     ...
     * @param GeneratorSettingsInterface $settings
     *
     * @return LandmapGenerator
     */
    public static function createFromSettings(GeneratorSettingsInterface $settings) : self
    {
     ...
     * @return SurfaceMap
     */
    public function generateMap() : SurfaceMap
    {
        $width = $this->surfaceMap->width;
        $height = $this->surfaceMap->height;
        ...
        for ($y = 0; $y < $height; $y++)
        {
            ...
            for ($x = 0; $x < $width; $x++)
            {
                ...
                if ($x % 2 == 0 && $y % 2 == 0)
                {
                    $this->surfaceMap->altitudes[$x][$y] = $this->getAltitude();
                }
            }
        }
